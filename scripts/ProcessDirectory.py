...
class CaseResults:
    def __init__(self):
        ...
        self.toolDataDf = pd.DataFrame
        self.timeDf = pd.DataFrame
        self.referenceDf = pd.DataFrame
...
def evaluateVariableResults(variable, timeColumnRef, timeColumnData, refData, testData, start, end, weightFactors):
    ...
    try:
        cr.timeDf = pd.DataFrame(data=pd.date_range(start="2018-01-01", periods=len(timeColumnRef), freq="H"),
                                 index=timeColumnRef, columns=["Date and Time"])
        cr.toolDataDf = pd.DataFrame(data=testData, index=timeColumnData, columns=["Data"])
        cr.referenceDf = pd.DataFrame(data=refData, index=timeColumnRef, columns=["Data"])
    ...
    pdTime = cr.timeDf.loc[start:end]
    pdData = cr.toolDataDf.loc[start:end]
    pdRef = cr.referenceDf.loc[start:end]
